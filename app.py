...
HF_TOKEN   = os.getenv("HF_TOKEN")                  # injected by Cloud Run
...
def text_to_speech(
    ...
    try:
        ...
        if hasattr(audio, "shape"):            # ndarray path
            waveform = audio
        else:                                  # generator path → concat chunks
            waveform = np.concatenate(list(audio), axis=1)

        buf = io.BytesIO()
        sf.write(buf, waveform.T, samplerate=24_000, format="WAV")
        buf.seek(0)
